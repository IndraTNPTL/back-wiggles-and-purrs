...
mongoose
	...
	.then(async (x) => {
		try {
			...
			await FoundAPet.deleteMany();

			const newFoundAPets = await FoundAPet.create(FoundAPets);
			console.log(newFoundAPets);

			process.exit();
		} catch (error) {
			console.error("Unable to create new Found a pet.");
			console.log(error);
		}
	})
	.catch((err) => {
